...
fn main() -> i32 {
    ...
    // Memory map the framebuffer (800x600 * 4 bytes/pixel)
...
fn draw_tangram(fb: *mut u8) {
    ...
    for &(points, color) in TANGRAM_POLYGONS {
        // Find bounding box for performance optimization
        ...
        for y in min_y..=max_y {
            for x in min_x..=max_x {
                // Only draw if the pixel is actually inside the tangram piece

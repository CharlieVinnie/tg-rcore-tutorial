// 8x8 ASCII Font (Chars 32 to 126: Space to '~')
pub const FONT_8X8: [[u8; 8]; 95] = [
    [0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00], // 32: Space
    [0x18,0x3C,0x3C,0x18,0x18,0x00,0x18,0x00], // 33: !
    [0x66,0x66,0x24,0x00,0x00,0x00,0x00,0x00], // 34: "
    [0x6C,0x6C,0xFE,0x6C,0xFE,0x6C,0x6C,0x00], // 35: #
    [0x18,0x3E,0x60,0x3C,0x06,0x7C,0x18,0x00], // 36: $
    [0x00,0x66,0x6C,0x18,0x30,0x66,0x00,0x00], // 37: %
    [0x38,0x6C,0x38,0x76,0xDC,0xCC,0x76,0x00], // 38: &
    [0x18,0x18,0x30,0x00,0x00,0x00,0x00,0x00], // 39: '
    [0x0C,0x18,0x30,0x30,0x30,0x18,0x0C,0x00], // 40: (
    [0x30,0x18,0x0C,0x0C,0x0C,0x18,0x30,0x00], // 41: )
    [0x00,0x66,0x3C,0xFF,0x3C,0x66,0x00,0x00], // 42: *
    [0x00,0x18,0x18,0x7E,0x18,0x18,0x00,0x00], // 43: +
    [0x00,0x00,0x00,0x00,0x00,0x18,0x18,0x30], // 44: ,
    [0x00,0x00,0x00,0x7E,0x00,0x00,0x00,0x00], // 45: -
    [0x00,0x00,0x00,0x00,0x00,0x18,0x18,0x00], // 46: .
    [0x06,0x0C,0x18,0x30,0x60,0xC0,0x80,0x00], // 47: /
    [0x3C,0x66,0x6E,0x76,0x66,0x66,0x3C,0x00], // 48: 0
    [0x18,0x38,0x18,0x18,0x18,0x18,0x7E,0x00], // 49: 1
    [0x3C,0x66,0x06,0x1C,0x30,0x60,0xFE,0x00], // 50: 2
    [0x3C,0x66,0x06,0x1C,0x06,0x66,0x3C,0x00], // 51: 3
    [0x0C,0x1C,0x3C,0x6C,0xFE,0x0C,0x0C,0x00], // 52: 4
    [0xFE,0x60,0x7C,0x06,0x06,0x66,0x3C,0x00], // 53: 5
    [0x3C,0x60,0x7C,0x66,0x66,0x66,0x3C,0x00], // 54: 6
    [0xFE,0x06,0x0C,0x18,0x30,0x30,0x30,0x00], // 55: 7
    [0x3C,0x66,0x3C,0x66,0x66,0x66,0x3C,0x00], // 56: 8
    [0x3C,0x66,0x66,0x66,0x3E,0x06,0x3C,0x00], // 57: 9
    [0x00,0x18,0x18,0x00,0x00,0x18,0x18,0x00], // 58: :
    [0x00,0x18,0x18,0x00,0x00,0x18,0x18,0x30], // 59: ;
    [0x06,0x0C,0x18,0x30,0x18,0x0C,0x06,0x00], // 60: <
    [0x00,0x00,0x7E,0x00,0x7E,0x00,0x00,0x00], // 61: =
    [0x60,0x30,0x18,0x0C,0x18,0x30,0x60,0x00], // 62: >
    [0x3C,0x66,0x0C,0x18,0x18,0x00,0x18,0x00], // 63: ?
    [0x3C,0x66,0x6E,0x6E,0x60,0x60,0x3C,0x00], // 64: @
    [0x3C,0x66,0x66,0x7E,0x66,0x66,0x66,0x00], // 65: A
    [0x7C,0x66,0x66,0x7C,0x66,0x66,0x7C,0x00], // 66: B
    [0x3C,0x66,0x60,0x60,0x60,0x66,0x3C,0x00], // 67: C
    [0x78,0x6C,0x66,0x66,0x66,0x6C,0x78,0x00], // 68: D
    [0x7E,0x60,0x60,0x78,0x60,0x60,0x7E,0x00], // 69: E
    [0x7E,0x60,0x60,0x78,0x60,0x60,0x60,0x00], // 70: F
    [0x3C,0x66,0x60,0x6E,0x66,0x66,0x3E,0x00], // 71: G
    [0x66,0x66,0x66,0x7E,0x66,0x66,0x66,0x00], // 72: H
    [0x3C,0x18,0x18,0x18,0x18,0x18,0x3C,0x00], // 73: I
    [0x06,0x06,0x06,0x06,0x66,0x66,0x3C,0x00], // 74: J
    [0x66,0x6C,0x78,0x70,0x78,0x6C,0x66,0x00], // 75: K
    [0x60,0x60,0x60,0x60,0x60,0x60,0x7E,0x00], // 76: L
    [0x63,0x77,0x7F,0x6B,0x63,0x63,0x63,0x00], // 77: M
    [0x66,0x76,0x7E,0x7E,0x6E,0x66,0x66,0x00], // 78: N
    [0x3C,0x66,0x66,0x66,0x66,0x66,0x3C,0x00], // 79: O
    [0x7C,0x66,0x66,0x7C,0x60,0x60,0x60,0x00], // 80: P
    [0x3C,0x66,0x66,0x66,0x6A,0x6C,0x3A,0x00], // 81: Q
    [0x7C,0x66,0x66,0x7C,0x6C,0x66,0x66,0x00], // 82: R
    [0x3C,0x66,0x60,0x3C,0x06,0x66,0x3C,0x00], // 83: S
    [0x7E,0x18,0x18,0x18,0x18,0x18,0x18,0x00], // 84: T
    [0x66,0x66,0x66,0x66,0x66,0x66,0x3C,0x00], // 85: U
    [0x66,0x66,0x66,0x66,0x66,0x3C,0x18,0x00], // 86: V
    [0x63,0x63,0x6B,0x7F,0x77,0x63,0x63,0x00], // 87: W
    [0x66,0x66,0x3C,0x18,0x3C,0x66,0x66,0x00], // 88: X
    [0x66,0x66,0x66,0x3C,0x18,0x18,0x18,0x00], // 89: Y
    [0x7E,0x06,0x0C,0x18,0x30,0x60,0x7E,0x00], // 90: Z
    [0x3C,0x30,0x30,0x30,0x30,0x30,0x3C,0x00], // 91: [
    [0x00,0x60,0x30,0x18,0x0C,0x06,0x00,0x00], // 92: \
    [0x3C,0x0C,0x0C,0x0C,0x0C,0x0C,0x3C,0x00], // 93: ]
    [0x18,0x3C,0x66,0x00,0x00,0x00,0x00,0x00], // 94: ^
    [0x00,0x00,0x00,0x00,0x00,0x00,0xFF,0x00], // 95: _
    [0x30,0x18,0x0C,0x00,0x00,0x00,0x00,0x00], // 96: `
    [0x00,0x00,0x3C,0x06,0x3E,0x66,0x3E,0x00], // 97: a
    [0x60,0x60,0x7C,0x66,0x66,0x66,0x7C,0x00], // 98: b
    [0x00,0x00,0x3C,0x66,0x60,0x66,0x3C,0x00], // 99: c
    [0x06,0x06,0x3E,0x66,0x66,0x66,0x3E,0x00], // 100: d
    [0x00,0x00,0x3C,0x66,0x7E,0x60,0x3C,0x00], // 101: e
    [0x1C,0x30,0x30,0x7C,0x30,0x30,0x30,0x00], // 102: f
    [0x00,0x00,0x3E,0x66,0x66,0x3E,0x06,0x3C], // 103: g
    [0x60,0x60,0x7C,0x66,0x66,0x66,0x66,0x00], // 104: h
    [0x18,0x00,0x38,0x18,0x18,0x18,0x3C,0x00], // 105: i
    [0x0C,0x00,0x1C,0x0C,0x0C,0x0C,0x0C,0x38], // 106: j
    [0x60,0x60,0x66,0x6C,0x78,0x6C,0x66,0x00], // 107: k
    [0x38,0x18,0x18,0x18,0x18,0x18,0x3C,0x00], // 108: l
    [0x00,0x00,0xEC,0xFE,0xD6,0xD6,0xD6,0x00], // 109: m
    [0x00,0x00,0x7C,0x66,0x66,0x66,0x66,0x00], // 110: n
    [0x00,0x00,0x3C,0x66,0x66,0x66,0x3C,0x00], // 111: o
    [0x00,0x00,0x7C,0x66,0x66,0x7C,0x60,0x60], // 112: p
    [0x00,0x00,0x3E,0x66,0x66,0x3E,0x06,0x06], // 113: q
    [0x00,0x00,0x7C,0x66,0x60,0x60,0x60,0x00], // 114: r
    [0x00,0x00,0x3E,0x60,0x3C,0x06,0x7C,0x00], // 115: s
    [0x30,0x30,0x7C,0x30,0x30,0x30,0x1C,0x00], // 116: t
    [0x00,0x00,0x66,0x66,0x66,0x66,0x3E,0x00], // 117: u
    [0x00,0x00,0x66,0x66,0x66,0x3C,0x18,0x00], // 118: v
    [0x00,0x00,0xC3,0xC3,0xD7,0xFE,0x6C,0x00], // 119: w
    [0x00,0x00,0x66,0x3C,0x18,0x3C,0x66,0x00], // 120: x
    [0x00,0x00,0x66,0x66,0x66,0x3E,0x06,0x3C], // 121: y
    [0x00,0x00,0x7E,0x0C,0x18,0x30,0x7E,0x00], // 122: z
    [0x0E,0x18,0x18,0x70,0x18,0x18,0x0E,0x00], // 123: {
    [0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x00], // 124: |
    [0x70,0x18,0x18,0x0E,0x18,0x18,0x70,0x00], // 125: }
    [0x00,0x00,0x00,0x32,0x4C,0x00,0x00,0x00], // 126: ~
];

pub struct Display {
    pub ptr: *mut u32,
    pub w: usize,
    pub h: usize,
}

impl Display {
    pub fn fill_rect(&self, x: usize, y: usize, w: usize, h: usize, color: (u8, u8, u8)) {
        let pixel_color: u32 = 0xFF000000 | ((color.0 as u32) << 16) | ((color.1 as u32) << 8) | (color.2 as u32);
        for dy in 0..h {
            if y + dy >= self.h { break; }
            for dx in 0..w {
                if x + dx >= self.w { break; }
                let offset = (y + dy) * self.w + (x + dx);
                unsafe { self.ptr.add(offset).write_volatile(pixel_color); }
            }
        }
    }

    pub fn draw_char(&self, x: usize, y: usize, c: u8, color: (u8, u8, u8), scale: usize) {
        // Now directly bounds checks the full 32-126 range instead of forcing uppercase
        if c < 32 || c > 126 { return; }     
        
        let bitmap = &FONT_8X8[(c - 32) as usize];
        
        for row in 0..8 {
            for col in 0..8 {
                if (bitmap[row] & (1 << (7 - col))) != 0 {
                    self.fill_rect(x + col * scale, y + row * scale, scale, scale, color);
                }
            }
        }
    }

    pub fn draw_string(&self, x: usize, y: usize, s: &str, color: (u8, u8, u8), scale: usize) {
        let mut cx = x;
        for b in s.bytes() {
            self.draw_char(cx, y, b, color, scale);
            cx += 8 * scale; 
        }
    }

    pub fn draw_number(&self, x: usize, y: usize, mut num: u32, color: (u8, u8, u8), scale: usize) {
        if num == 0 {
            self.draw_char(x, y, b'0', color, scale);
            return;
        }
        
        let mut digits = [0u8; 10];
        let mut count = 0;
        while num > 0 {
            digits[count] = (num % 10) as u8 + b'0';
            num /= 10;
            count += 1;
        }
        
        let mut cx = x;
        for i in (0..count).rev() {
            self.draw_char(cx, y, digits[i], color, scale);
            cx += 8 * scale;
        }
    }
}
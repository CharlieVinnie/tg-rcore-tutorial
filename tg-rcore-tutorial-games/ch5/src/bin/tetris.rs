...
fn main() -> i32 {
    ...
    let offset_y = screen_h.saturating_sub(play_h) / 2;
    ...
    let info_x = offset_x + play_w + 30;
    let hold_x = offset_x.saturating_sub(130);
    let info_y = offset_y;
    ...
    loop {
        ...
        // Rendering Delta
        if force_redraw {
            display.fill_rect(offset_x - 1, offset_y - 1, play_w + 2, play_h + 2, COLOR_BORDER);
            ...
            for y in 0..BOARD_H {
                for x in 0..BOARD_W {
                    ...
                    display.fill_rect(px + inset, py + inset, CELL_SIZE - 2 * inset, CELL_SIZE - 2 * inset, color);
                    if t == 0 { display.fill_rect(px + CELL_SIZE/2, py + CELL_SIZE/2, 1, 1, COLOR_GRID); }
                }
            }
                ...
                for y in 0..4 {
                    for x in 0..4 {
                        if piece_at(current_t, current_rot, x, y) {
                            ...
                            if ghost_y >= 0 {
                                let g_px = offset_x + (cx + x) as usize * CELL_SIZE;
                                let g_py = offset_y + (ghost_y + y) as usize * CELL_SIZE;
                                display.fill_rect(g_px + 2, g_py + 2, CELL_SIZE - 4, CELL_SIZE - 4, (block_color.0/3, block_color.1/3, block_color.2/3));
                            }
                            
                            if cy + y >= 0 {
                                let a_px = offset_x + (cx + x) as usize * CELL_SIZE;
                                let a_py = offset_y + (cy + y) as usize * CELL_SIZE;
                                display.fill_rect(a_px + 1, a_py + 1, CELL_SIZE - 2, CELL_SIZE - 2, block_color);
                            }
                        }
                    }
                }
            ...
            for y in 0..4 {
                for x in 0..4 {
                    if piece_at(next_t, 0, x, y) {
                        display.fill_rect(info_x + x as usize * CELL_SIZE + 1, info_y + 30 + y as usize * CELL_SIZE + 1, CELL_SIZE - 2, CELL_SIZE - 2, COLORS[next_t as usize]);
                    }
                }
            }
                ...
                for y in 0..4 {
                    for x in 0..4 {
                        if piece_at(hold_t, 0, x, y) {
                            display.fill_rect(hold_x + x as usize * CELL_SIZE + 1, info_y + 30 + y as usize * CELL_SIZE + 1, CELL_SIZE - 2, CELL_SIZE - 2, h_color);
                        }
                    }
                }
            }
            ...
            if game_over {
                display.draw_string(offset_x + 30, offset_y + play_h / 2, "GAME OVER", COLOR_BORDER, 3);
            }

...
run_base() {
    ...
    if cargo run 2>&1 | tee /dev/stderr | tg-rcore-tutorial-checker --ch 6; then
        echo ""
...
run_exercise() {
    ...
    if cargo run --features exercise 2>&1 | tee /dev/stderr | tg-rcore-tutorial-checker --ch 6 --exercise; then
        echo ""
        echo -e "${YELLOW}────────── 测试结果 ──────────${NC}"

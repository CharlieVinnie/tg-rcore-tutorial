...
pub use area::MapVisibility;
use crate::{PageManager, space::area::MappedRange};
use alloc::vec::Vec;
...
pub struct AddressSpace<Meta: VmMeta, M: PageManager<Meta>> {
    ...
    pub areas: Vec<MappedRange<Meta>>,
    page_manager: M,
}
...
impl<Meta: VmMeta, M: PageManager<Meta>> AddressSpace<Meta, M> {
    ...
    /// 向地址空间增加映射关系。
    pub fn map_extern(&mut self, range: Range<VPN<Meta>>, pbase: PPN<Meta>, flags: VmFlags<Meta>, visibility: MapVisibility) {
        // map_extern 假设物理页已由外部准备好，此处只负责建立页表项。
        self.areas.push(MappedRange::new(range.start..range.end, visibility));
    ...
    pub fn map(
        ...
        self.map_extern(range, self.page_manager.v_to_p(page), flags, visibility)
    }
        ...
        for area in self.areas.drain(..) {
            if area.end() <= range.start || area.start() >= range.end {
                ...
                if area.start() < range.start {
                    new_areas.push(MappedRange::new(area.start()..range.start, area.visibility));
                }
                if area.end() > range.end {
                    new_areas.push(MappedRange::new(range.end..area.end(), area.visibility));
                }
            }
        }
    ...
    /// 遍历地址空间，将其中的地址映射添加进自己的地址空间中，对于 PRIVATE 映射，重新分配物理页并拷贝所有数据及代码；对于 SHARED 映射，直接建立映射关系（不拷贝）。
        ...
        for (_, range) in areas.iter().enumerate() {
            ...
            match range.visibility {
                ...
                MapVisibility::SHARED => {
                    ppn = self.page_manager.v_to_p(data_ptr);
                }
            }
            new_addrspace.map_extern(vpn_range, ppn, flags, range.visibility);
        }
    }
}
